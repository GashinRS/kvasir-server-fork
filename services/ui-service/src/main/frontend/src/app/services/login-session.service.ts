...
export class LoginSessionService {
  ...
  private readonly tempStorage = sessionStorage;
  ...
  /**
   * Update podName of stored Login session.
   * Usefull after different login calls back than what pod was selected (eg. select pod alice, login with bob)
   * @param podName
   */
  updateCurrentLoginSession(podName: string) {
    let current = this.getCurrentLoginSession();
    if (current != null) {
      current.podName = podName;
    }
    this.persistLoginSession();
  }
