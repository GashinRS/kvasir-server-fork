...
export class LoginFSM {
  ...
  private async executeCurrentState(
    ...
    switch (this.loginSessionService.getCurrentLoginState()) {
      ...
      case LoginState.PREPARE_LOGIN:
        ...
        if (podName == null) {
          throw new Error('podName argument was missing!');
        }
        if (redirectToRoot == null) {
          throw new Error('redirectToRoot argument was missing!');
        }
        if (force == null) {
          throw new Error('force argument was missing!');
        }
  ...
  private async triggerKeycloakLogin(): Promise<void> {
    if (this.kc) {
      ...
      const prompt = loginSession.force // this.loginSessionService.popForceFlag()
        ? 'login'
        : undefined;
      ...
      return this.kc.login({
        redirectUri: redirectToRoot
          ...
          : redirectUri,
        prompt,
        maxAge: prompt == undefined ? undefined : 240,
      });
    } else {
      console.error(
  ...
  private waitingForCallback(): void {
    ...
    this.session.activateSession(
      this.kc?.idTokenParsed?.['preferred_username'] ?? null,
  ...
  public async logout(redirectUri?: string) {
    ...
    this.kc?.logout({
      redirectUri: redirectUri ?? window.location.origin + '/_ui',
    });
