// neg-manage-and-existing.cue — illegal combination of `manage: true` with
// `existingSecret`. Expected: vet FAILS with "cannot be combined with `existingSecret`".

values: {
	applicationConfig: {
		kg: clickhouse: {
			host:     "clickhouse.example.svc.cluster.local"
			port:     8123
			user:     "test-user"
			password: "test-password"
		}

		messaging: kafka: "bootstrap-servers": "kafka.example.svc.cluster.local:9092"

		storage: s3: {
			endpoint: "http://s3.example.svc.cluster.local:8333"
			region:   "us-east-1"
		}

		auth: keycloak: {
			url:   "http://keycloak.example.svc.cluster.local:8080"
			realm: "kvasir"
			"admin-client": {
				"server-url": "http://keycloak.example.svc.cluster.local:8080"
				"realm":      "master"
			}
		}

		pep: openfga: url: "http://openfga.example.svc.cluster.local:8080"
	}

	secrets: clickhouse: {
		manage:         true
		existingSecret: "test-clickhouse"
	}
}
