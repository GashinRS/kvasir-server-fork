...
HTTP request to the `{podId}/query-events` endpoint with the `Accept: text/event-stream` header. This will return a
stream of Server-Sent Events (SSE), containing information on the queries that are being executed on the Pod in
near-realtime.

**GET** `http://localhost:8080/alice/query-events` with `Accept: text/event-stream`
...
> creating a Kafka consumer for the `query.requests` topic.
> {style="note"}

## Life-cycle events

You can subscribe to life-cycle events of the Pod (config changes, slice creation, etc), by sending an HTTP request to
the`{podId}/life-cycle-events` endpoint with the `Accept: text/event-stream` header. This will return a stream of
Server-Sent-Events (SSE), containing information on the life-cycle events of the Pod in near-realtime.

**GET** `http://localhost:8080/alice/life-cycle-events` with `Accept: text/event-stream`
...
> For services that have direct access to the Kvasir infrastructure: you can subscribe to the same information by
> creating a Kafka consumer for the `lifecycle.events` topic.
> {style="note"}

## Storage events

You can subscribe to storage events of the Pod (operations on the S3 store), by sending an HTTP request to the
`{podId}/s3-events` endpoint with the `Accept: text/event-stream` header. This will return a stream of
Server-Sent-Events (SSE), containing information on the performed S3 operations.

**GET** `http://localhost:8080/alice/s3-events` with `Accept: text/event-stream`
...
> For services that have direct access to the Kvasir infrastructure: you can subscribe to the same information by
> creating a Kafka consumer for the `storage.events` topic.
> {style="note"}